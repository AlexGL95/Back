...
export class PropuestaService {
    ...
    async obtenerPropuesta( categoria: number, area: number, pagina: number ): Promise<{ rcArr: Propuesta[], nSig: number }> {
        ...
        if( (categoria == 0) && (area == 0) ) {
            rcArr = await this.propuestaRepository.find( { order: { id: "DESC" }, relations: ['categoria', 'area'], skip: skip, take: 50 } );
        }
        // Si no tiene area pero si categoria = retorna la paginacion correspondiente a la categoria.
            ...
            rcArr = await this.propuestaRepository.find( { order: { id: "DESC" }, relations: ['categoria', 'area'], where: { categoria: categoriasArr[categoria - 1] }, skip: skip, take: 50 } );
        }
        // Si tiene area y categoria = retorna la paginacion correspondiente al area de la categoria.
            ...
            rcArr = await this.propuestaRepository.find( { order: { id: "DESC" }, relations: ['categoria', 'area'], where: { categoria: categoriasArr[categoria - 1], area: areasArr[area - 1] }, skip: skip, take: 50 } );
        }
        else {
