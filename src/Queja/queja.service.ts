...
export class QuejaService {
    ...
    async obtenerQueja( categoria: number, area: number, pagina: number ): Promise<{ rcArr: Queja[], nSig: number }> {
        ...
        if( (categoria == 0) && (area == 0) ) {
            rcArr = await this.quejaRepository.find( { order: { id: "DESC" }, skip: skip, take: 50 } );
        }
        // Si no tiene area pero si categoria = retorna la paginacion correspondiente a la categoria.
            ...
            rcArr = await this.quejaRepository.find( { order: { id: "DESC" }, where: { categoria: categoriasArr[categoria - 1] }, skip: skip, take: 50 } );
        }
        // Si tiene area y categoria = retorna la paginacion correspondiente al area de la categoria.
            ...
            rcArr = await this.quejaRepository.find( { order: { id: "DESC" }, where: { categoria: categoriasArr[categoria - 1], area: areasArr[area - 1] }, skip: skip, take: 50 } );
        }
        else {
